...
public sealed partial class RDStatusEffectSystem
{
    /// <summary>
    ...
    {
        if (!_prototype.TryIndex(effectProto, out var effectProtoData))
            return false;

        if (!effectProtoData.TryGetComponent<RDStatusEffectComponent>(out var effectProtoComp,
                _compFactory))
            return false;
        ...
        var ev = new RDBeforeStatusEffectAddedEvent(effectProto);
        ...
        bool resetCooldown = false)
    {
        ...
        EnsureComp<RDStatusEffectContainerComponent>(uid, out var container);
        ...
        var ev = new RDStatusEffectAppliedEvent(uid, (effect, effectComp));
        RaiseLocalEvent(uid, ref ev);
    ...
    public bool TryRemoveStatusEffect(EntityUid uid, EntProtoId effectProto)
    {
        ...
        foreach (var effect in container.ActiveStatusEffects)
        {
            ...
            if (meta.EntityPrototype is null || meta.EntityPrototype != effectProto)
                continue;

            if (!_effectQuery.TryComp(effect, out var effectComp))
                return false;

            var ev = new RDStatusEffectRemovedEvent(uid, (effect, effectComp));
            RaiseLocalEvent(uid, ref ev);
            RaiseLocalEvent(effect, ref ev);
    ...
    public bool TryGetTime(EntityUid uid,
        ...
        RDStatusEffectContainerComponent? container = null)
    {
        time = default;
